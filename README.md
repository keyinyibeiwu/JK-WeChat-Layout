...
![WeChat Compatible](https://img.shields.io/badge/WeChat-Compatible-success.svg)
[![JK Signature](https://img.shields.io/badge/JK-Signature-blue?style=flat-square)]
> 百变风格 · 全场景适配 | V3.0 JK Signature Edition
---
📖 项目简介
JK-WeChat-Layout 是一款基于 AI 大模型的智能排版技能插件。用户只需输入原始文字内容，AI 将自动分析内容语义与情感基调，并匹配最适合的排版风格，最终输出一键可用的微信 HTML 代码。
...
---
🚀 安装与使用
在 OpenClaw/QClaw 中加载技能
下载技能压缩包：
访问 https://github.com/keyinyibeiwu/JK-WeChat-Layout
点击绿色的 `Code` 按钮 → `Download ZIP`
在平台中导入：
打开 OpenClaw/QClaw 平台
在技能管理中选择「导入」或「添加技能」
选择下载的 ZIP 压缩包
或解压后选择文件夹
启用技能：
在技能列表中找到「JK-WeChat-Layout」
点击启用
快速使用示例
向 AI 发送以下格式的内容：
```plaintext
...
📦 输出产物与公众号使用流程
输出产物
本技能输出的产物为完整 HTML 网页文件，包含完整的排版内容和文档结构，可直接在浏览器中打开查看效果。
两步上稿（极简流程）
第一步：复制 HTML
